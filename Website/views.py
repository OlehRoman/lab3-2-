...
@api_view(['GET'])
def get_all_products_net(request):
    products = NetworkHelper.get_list("product")
    return Response(products)

@api_view(['GET'])
def get_product_net(request, id):
    product = NetworkHelper.get_item("product", id)
    return Response(product)

@api_view(['DELETE'])
def delete_product_net(request, id):
    success = NetworkHelper.delete_item("product", id)
    return Response({"success": success})


def object_list_view(request):
    objects = NetworkHelper.get_list("product")
    return render(request, "WebSite/object_list.html", {"objects": objects})

def delete_object(request, id):
    NetworkHelper.delete_item("product", id)
    return redirect("object_list")  # Повертаємось до списку після видалення
